...
export const isDesktop = isBrowser &&
    (window.process?.type === 'renderer' || !!window.electron || navigator.userAgent.includes('Electron'));

// Capacitor detection
export const isMobile = isBrowser &&
    (!!window.Capacitor || !!window.androidBridge || !!window.webkit?.messageHandlers?.bridge || navigator.userAgent.includes('Capacitor'));
