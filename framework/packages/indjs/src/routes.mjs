...
export function fileToRoute(pagesDir, file) {
  ...
  const noExt = rel.replace(/\.(jsx|js|mjs|tsx|ts)$/i, '').split('/').filter(s => !/^\(.+\)$/.test(s)).join('/');
...
export async function discoverRoutes(root) {
  ...
  try { await fs.access(pagesDir); } catch { return { pages: [], api: [] }; }
  const files = (await walk(pagesDir))
    .filter(f => PAGE_EXTS.includes(path.extname(f)))
    .filter(f => !path.basename(f).includes('__indjs.'));
  const pageFiles = files.filter(f => !fileToRoute(pagesDir, f).startsWith('/api/'));
  const apiFiles = files.filter(f => fileToRoute(pagesDir, f).startsWith('/api/'));
