...
function showHelp() {
  ...
  console.log(
    "  mobile    Mobile helpers (Capacitor): dev, build, sync, android, ios",
  );
  ...
  console.log(
    "  --webDir <path>   Optional output dir for built static assets (e.g. for Capacitor)",
  );
  console.log(
    "  --noPrompt        Skip interactive prompts in generators (use sensible defaults)",
  );
    ...
    switch (cmd) {
      ...
      case "g":
        ...
        return generate({
          ...
          name,
          root,
          noPrompt: !!(args.noPrompt || args.quick),
        });
      ...
      case "desktop": {
        ...
        if (!["dev", "start"].includes(sub)) {
          console.error(
            chalk.red("❌ Usage: indjs desktop <dev|start> [--port <number>]"),
          );
          process.exit(1);
        }
        const cmdStr =
      ...
      case "mobile": {
        ...
        if (sub === "dev") {
          ...
          try {
            ...
            if (!args.noConfig) {
              console.log(
                chalk.yellow(
                  "   → Updating capacitor.config.json server url...",
                ),
              );
              capConfig.server = {
                ...capConfig.server,
              ...
              console.log(
                chalk.yellow("   → Syncing config to Android/iOS..."),
              );
              await runShell("npx cap copy", { cwd: root });
            }
          } catch (e) {
            console.warn(
              "   ⚠️ Could not automatically update capacitor config:",
              e.message,
            );
          }
          ...
          console.log(
            chalk.green(
              `🚀 Launching ${target} app and Metro-like Dev Server...`,
            ),
          );
          console.log(
            chalk.cyan(
              "💡 To debug: Open chrome://inspect in your browser to inspect the WebView.",
            ),
          );
          ...
          setTimeout(() => {
            const native = spawn("npx", ["cap", "run", target], {
              stdio: "inherit",
              shell: true,
            });
            ...
            native.on("exit", (code) => {
              if (code !== 0)
                console.error(`Native run exited with code ${code}`);
        ...
        if (!cmdStr) {
          console.error(
            chalk.red("❌ Usage: indjs mobile <dev|build|sync|android|ios>"),
          );
          process.exit(1);
        }
        return runShell(cmdStr, { cwd: root });
      ...
      case "ai": {
        ...
        if (!sub) {
          console.error(
            chalk.red("❌ Usage: indjs ai <scaffold|docs|refactor> ..."),
          );
          process.exit(1);
        }
        if (sub === "scaffold") {
          ...
          if (!type || !name) {
            console.error(
              chalk.red("❌ Usage: indjs ai scaffold <type> <name>"),
            );
          ...
          await fs.writeFile(
            out,
            header + (ai || "Local AI unavailable. Falling back to generator."),
            "utf8",
          );
          console.log(chalk.green(`AI preview written to ${out}`));
          if (!ai) {
            console.log(
              chalk.blue(`Falling back to built-in generator: ${type} ${name}`),
            );
        ...
        console.error(
          chalk.red("❌ Unknown ai subcommand. Use: scaffold, docs, refactor"),
        );
        process.exit(1);
      }
