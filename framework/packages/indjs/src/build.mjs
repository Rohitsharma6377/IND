...
export async function build({ root, baseUrl, webDir }) {
  ...
  for (const r of pages) console.log(`   /${r.route.replace(/^\//, '')} → ${r.file}`);
  ...
  for (const p of pages) {
    try {
      ...
      for (const entry of paths) {
        ...
        let clientSrc = null;
        try {
          const manifestPath = path.join(root, '.indjs', 'client', 'manifest.json');
        ...
        const html = await renderPageModule({
          mod,
          ctx: { req: {}, res: {}, query: {}, params, root, pageFile: p.file, route: p.route, dev: false },
          assets: { clientSrc },
          props
        });
