  ...
  return (
    <div className="min-h-screen py-24 container mx-auto px-6">
      <div className="text-center mb-16 animate-fade-in-up">
        <h1 className="text-4xl md:text-5xl font-bold mb-6 text-slate-900 dark:text-white">Learning Path</h1>
      ...
      <div className="max-w-4xl mx-auto relative">
        {/* Vertical Line */}
        <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-1 bg-slate-200 dark:bg-white/10 -translate-x-1/2 hidden md:block" />
          ...
          {steps.map((step, i) => (
            <div key={i} className={`flex flex-col md:flex-row items-center gap-8 group animate-fade-in-up`} style={{ animationDelay: `${i * 100}ms` }}>
              ...
              <div className={`flex-1 w-full ${i % 2 === 0 ? 'md:text-right' : 'md:order-last'}`}>
                <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/5 p-6 rounded-2xl hover:border-indigo-500/50 transition-colors shadow-sm dark:shadow-none">
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{step.title}</h3>
                  <p className="text-slate-600 dark:text-zinc-400 mb-4">{step.desc}</p>
                </div>
              </div>

              {/* Center Node */}
              <div className={`w-8 h-8 rounded-full ${step.color} border-4 border-slate-50 dark:border-[#030712] relative z-10 shadow-[0_0_20px_currentColor] hidden md:block`} />

              {/* Code Right (Even) or Left (Odd) */}
              <div className={`flex-1 w-full ${i % 2 === 0 ? '' : 'md:text-right'}`}>
                <div className="font-mono text-sm bg-slate-900 dark:bg-black/50 p-4 rounded-xl border border-slate-200 dark:border-white/10 text-indigo-300 shadow-inner">
                  $ {step.code}
                </div>
              </div>
            </div>
      ...
      <div className="text-center mt-24 animate-fade-in-up animation-delay-500">
        <a href="/docs" className="inline-flex items-center gap-2 text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-white transition-colors">
