    ...
    try {
      ...
      if (url.pathname.startsWith('/api')) {
        ...
        const resShim = {
          ...
          status(code){ this._status = code; },
          setHeader(k,v){ this._headers.set(k,v); },
          json(obj){ return new Response(JSON.stringify(obj), { status: this._status, headers: this._headers }); },
          end(text){ return new Response(text, { status: this._status, headers: this._headers }); },
          get headersSent(){ return false; }
        };
        const fn = mod[method] || mod.default;
        if (!fn) return json({ error: 'Method Not Allowed' }, 405);
        const result = await fn({ req: request, res: resShim, params: match.params, query: Object.fromEntries(url.searchParams), body });
      ...
      const resShim = {
        ...
        status(code){ this._status = code; },
        setHeader(k,v){ this._headers.set(k,v); },
        end(html){ return new Response(html, { status: this._status, headers: this._headers }); },
        get headersSent(){ return false; }
      };
      const rendered = await renderPageModule({ mod, ctx: { req: request, res: resShim, query: Object.fromEntries(url.searchParams), params: match.params, root, pageFile: match.route.file, route: match.route.route }, assets: { clientSrc } });
